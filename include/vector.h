...
namespace leistd {
    ...
    public:
        ...
        public:
            ...
            reverse_iterator operator-(difference_type n) const { return reverse_iterator(ptr + n); }
            difference_type operator-(const reverse_iterator &rhs) const { return rhs.ptr - ptr; }
        ...
        reverse_iterator crbegin() const { return reverse_iterator(_end); }
        reverse_iterator crend() const { return reverse_iterator(_start); }
        ...
        iterator insert(const_iterator pos, T &&value) {
            ...
            std::move_backward(_start + idx, _end, _end + 1);
            // 插入元素
            std::allocator_traits<Alloc>::construct(_alloc, _start + idx, std::move(value));
            ++_end;
            return iterator(_start + idx);
        }
        ...
        iterator insert(const_iterator pos, InputIt first, InputIt last) {
            ...
            if (n == 0) return begin() + (pos - cbegin());
...
namespace leistd {
    ...
    void vector<T, Alloc>::push_back(T &&value) {
        if (_end == _cap) reserve(capacity() == 0 ? 1 : capacity() * 2);
    ...
    vector<T, Alloc>::~vector() {
        clear();
        if (_start) std::allocator_traits<Alloc>::deallocate(_alloc, _start, capacity());
    }
    ...
    typename vector<T, Alloc>::const_reference vector<T, Alloc>::at(size_type index) const {
        if (index >= size()) throw std::out_of_range("vector");
        return _start[index];
    }
    ...
    bool vector<T, Alloc>::operator==(const vector &other) const {
        ...
        for (size_type i = 0; i < size(); ++i)
            if (_start[i] != other._start[i]) return false;
        return true;
    }
    ...
    // 拷贝构造
    template<typename T, typename Alloc>
    vector<T, Alloc>::vector(const vector &other): _start(nullptr), _end(nullptr), _cap(nullptr) {
        reserve(other.size());
        for (pointer p = other._start; p != other._end; ++p) {
            std::allocator_traits<Alloc>::construct(_alloc, _end, *p);
            ++_end;
    ...
    vector<T, Alloc>::vector(vector &&other) noexcept
        : _alloc(std::move(other._alloc)), _start(other._start), _end(other._end), _cap(other._cap) {
        other._start = other._end = other._cap = nullptr;
    }
}
